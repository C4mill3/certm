...
openssl req -key $CA_KEY -new -x509 -out $CA_CERT -days 3650 -config "config/default_ca.cnf" -subj "/C=FR/O=Organization/CN=My CA"
...
openssl req -new -key $SERVER_KEY -out $SERVER_CSR -subj "/C=FR/O=Organization/CN=www.example.com"
# Signer le certificat du serveur avec la CA
openssl x509 -req -in $SERVER_CSR -CA $CA_CERT -CAkey $CA_KEY -CAcreateserial -out $SERVER_CERT -days 365 -extfile "config/default_x509.cnf"
